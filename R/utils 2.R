...
profileCI <- function(nll, whichPar, MLE, interval, level) {
  ...
  lower <- tryCatch(
    uniroot(f, c(interval[1], MLE[whichPar]))$root,
  ...
  upper <- tryCatch(
    ...
    error = function(e) {
      warning(
        "Upper endpoint of profile confidence interval is on the boundary.",
...
csvToUMF <-
  function(filename, long = FALSE, type, species = NULL, ...) {
    ...
    if (long == TRUE) {
      return(formatLong(dfin, species, type = type, ...))
    } else {
      return(formatWide(dfin, type = type, ...))
    }
  }
...
formatWide <-
  function(dfin, sep = ".", obsToY, type, ...) {
    ...
    while (i <= ncols) {
      ...
      if (
        !identical(
          grep(paste(sep.reg, "[[:digit:]]+$", sep = ""), dfnm[i]),
          integer(0)
        )
      ) {
        # check if this is obsdata

        newvar.name <- sub(
          paste(sep.reg, "[[:digit:]]+$", sep = ""),
          '',
          dfnm[i]
        )

        newvar <- dfin[, grep(
          paste(newvar.name, sep.reg, "[[:digit:]]+$", sep = ""),
          dfnm
        )]
        ...
        if (obsCovsPresent) {
          if (ncol(newvar) != R) {
            stop(
              "Not all observation-level covariates have the same number of columns."
            )
          } else {
...
formatMult <-
  function(df.in) {
    ...
    obsvars.df2 <- as.data.frame(obsvars.df[,
      !(names(obsvars.df) %in% names(siteCovs))
    ])
        ...
        for (k in 1:J) {
          if (obs.i.missing[j, k]) {
            if (all(is.na(obs.i.mat[j, ]))) {
              ...
              obs.i.imputed[j, k] <- mean(c(
                mean(obs.i.mat[j, ], na.rm = T),
  ...
  switch(
    ...
    abund = {
      ...
      if (!isTRUE(all.equal(ndi, 1))) {
        stop(
          "output cannot equal 'abund' when distance intervals differ. Use output='density' instead."
        )
      }
      ...
      switch(
        ...
        line = {
          ...
          if (!isTRUE(all.equal(ntl, 1))) {
            stop(
              "output cannot equal 'abund' when transect lengths differ. Use output='density' instead."
            )
          }
