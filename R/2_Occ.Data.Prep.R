...
occupancy_data_prep <- function(cft_inspection, scs_ontology_reshaped){
  #----Set Directories----
  ...
  cft_inspection$db.source <- "tt"

  scs_ontology_reshaped$db.source <- "scs"
  ...
  scs_ontology_reshaped <- completeFun(scs_ontology_reshaped, col.names)
  dim(scs_ontology_reshaped)
  names(scs_ontology_reshaped)

  #--Merge SCS and Tick Tracker Data
  dat <- rbind.data.frame(cft_inspection, scs_ontology_reshaped)
  dim(dat)
  names(dat)
