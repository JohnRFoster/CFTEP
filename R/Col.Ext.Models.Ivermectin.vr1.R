...
file.name <- "dat.occ.cov.ivermectin.2022-05-10.csv"
...
remove.vec<-sample(cnt<-cnt[cnt$freq==1,"x"], size=length(cnt)*.80,replace=FALSE)
remove.vec<-cnt<-cnt[cnt$freq<4,"x"]
...
col.names<-col.names[col.names %!in% c("pasture_name","y","year")]

occ.dat<-occ.dat[,c("year","pasture_name","y",col.names)]


#---- Set Factors and Reference Categories ----

#--Add county grouping variable
tmp<-plyr::count(occ.dat$county_name)
tmp[order(tmp$freq),]
...
occ.dat <- within(occ.dat, cnty.index <- relevel(cnty.index, ref=3))
...
saveRDS(occ.dat, file=paste0(model.dat.path,"formated.occ.dat.ivermectin.",Sys.Date(),".RDS"))

#occ.dat<-readRDS(paste0(model.dat.path,"formated.occ.dat.ivermectin.2022-05-27.RDS"))




occ.dat$corn.binary <-0
occ.dat[occ.dat$site.qty_corn>0,"corn.binary"] <- 1

cnt<-plyr::count(occ.dat[,c("year","pasture_name","y","corn.binary")])

cnt<-plyr::count(cnt[,c("year","y","corn.binary")])
...
col.names[col.names %!in% names(obsCovs(occ.dat))]

col.names<-c("qty_inspected","qty_infested","month",
             "apr.infest.prev","prop.new.animals")
...
names(obsCovs(occ.dat))



#Sites
col.names<-c("site.pasture_qty_acres",
             "site.mean.pas.dist","site.sd.pas.dist",
             "site.med.dist.pos","site.sd.dist.pos","site.num.adj.pas",
             "site.lag.count","site.lag.rate","mx.distance",
             "site.qty_corn","site.qty_totalcorndays",
             "site.qty_cornfeeders","site.feeder_density")

col.names[col.names %!in% names(siteCovs(occ.dat))]

col.names<-c("site.qty_corn","site.qty_totalcorndays",
             "site.qty_cornfeeders","site.feeder_density")
...
col.vars<-c("county_name")
ext.vars<-c("site.qty_corn.std","site.feeder_density.std")
p.vars<-c("qty_inspected.std","species","inspection_type","month")
...
tmp<-siteCovs(occ.dat)

plot(tmp[,c("site.qty_corn.std","site.qty_totalcorndays.std","site.qty_cornfeeders.std","site.feeder_density.std")])

corr(tmp[,c("site.qty_corn.std","site.qty_totalcorndays.std")])
corr(tmp[,c("site.qty_corn.std","site.qty_cornfeeders.std")])
corr(tmp[,c("site.qty_corn.std","site.feeder_density.std")])
...
m.full <- stan_colext(~1, ~1, ~1, ~1, data=occ.dat, chains=3, iter=300)
...
mod <- nonparboot(mod, B=1)  # This takes a while and should be at min 1000 for final model!
...
mod<-readRDS(paste0(model.results.path,"fit.col.ext.model.2021-08-18.RDS"))
...
write.csv(equil.occ, paste0(write.path,"Equilibrium.occupancy.",Sys.Date(),".csv"), row.names=FALSE)
write.csv(occ.est, paste0(write.path,"occupancy.probability.population.",Sys.Date(),".csv"), row.names=FALSE)
...
scratch.detection.preds <-Reduce(rbind.data.frame, list(det.cattle.preds, det.nilgai.preds, det.deer.preds, det.equine.preds))


write.path <- "C:/Documents/Project Documents/CattleFeverTick/Model.Results/"

write.csv(scratch.detection.preds, paste0(write.path, "scratch.detection.predictions.",Sys.Date(),".csv"))
write.csv(preds, paste0(write.path, "all.predictions.",Sys.Date(),".csv"))
...
write.csv(det.preds, paste0(write.path, "mean.detection.cnty.year.",Sys.Date(),".csv"))
write.csv(col.preds, paste0(write.path, "mean.colinization.cnty.year.",Sys.Date(),".csv"))
write.csv(ext.preds, paste0(write.path, "mean.exinction.cnty.year.",Sys.Date(),".csv"))
...
write.path<-"G:/DiseaseSpecificProjects/CattleFeverTick/AnalyticalFrameworkCFT2021/Results/Model.Predictions/"

write.csv(det.site.mu.all, paste0(write.path,"mean.pasture.level.detection.probability.17Sep2021.csv"), row.names = FALSE)
