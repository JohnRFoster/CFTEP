...
reformat_ontology <- function(cftep_dis_data, cft_inspection){
  ...
  tmplate_dat <- read_csv(cft_inspection)
  tmplate_dat <- tmplate_dat[FALSE, ]

  #Get column names
  template_cols <- colnames(tmplate_dat)
  col_names <- template_cols[!template_cols %in% colnames(dat_qty)]

  #Generate template data
  tmplate_dat <- tmplate_dat[, colnames(tmplate_dat) %in% col_names]

  #Add NA values
  tmplate_dat[seq_len(nrow(dat_qty)), ] <- NA

  #Add to SCS data
  dat <- bind_cols(dat_qty, tmplate_dat)
